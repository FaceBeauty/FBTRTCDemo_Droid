...
//       showPanel(FBViewState.STICKER);//贴纸
  ...
  //      case HIDE:
        case AR_WATERMARK:
          RxBus.get().post(FBEventAction.ACTION_SHOW_WATERMARK_TAKEPHOTO,"");
          hideContainer();
          break;
        case BEAUTY:
        case STICKER:
  ...
  //        break;
     ...
     */
    @Subscribe(thread = EventThread.MAIN_THREAD,
            tags = { @Tag(FBEventAction.ACTION_ADD_STICKER_RECT) })
    ...
    private void initWatermark() {
      if(FBSelectedPosition.POSITION_WATERMARK > 0){
        float[] points = FBSelectedPosition.WATERMARK_POINTS;
        if(points != null){
