...
import com.nimo.fb_effect.adapter.LightMakeupItemViewBinder;
 ...
 * 轻彩妆
 */
public class LightMakeupItemFragment extends FBBaseLazyFragment {

    private View container;

    private RecyclerView rvlightMakeUp;

    private final List<LightMakeupConfig.LightMakeup> items = new ArrayList<>();
    ...
    @Override protected void initView(View view, Bundle savedInstanceState) {
        ...
        rvlightMakeUp = findViewById(R.id.rv_filter);

        rvlightMakeUp.addItemDecoration(new
                MyItemDecoration(5)
        );
        ...
        LightMakeupConfig lightMakeupList = FBConfigTools.getInstance().getLightMakeupList();

        if (lightMakeupList == null) {
            FBConfigTools.getInstance().getLightMakeupConfig(new FBConfigCallBack<List<LightMakeupConfig.LightMakeup>>() {
                @Override public void success(List<LightMakeupConfig.LightMakeup> list) {
                    items.addAll(list);
                    initRecyclerView();
                }
            ...
            items.addAll(lightMakeupList.getLight_makeup());
            initRecyclerView();
        }
    ...
    public void initRecyclerView() {
        listAdapter.register(LightMakeupConfig.LightMakeup.class, new LightMakeupItemViewBinder());
        listAdapter.setItems(items);
        rvlightMakeUp.setAdapter(listAdapter);
        rvlightMakeUp.smoothScrollToPosition(FBUICacheUtils.getLightMakeupPosition());
        //sync progress
