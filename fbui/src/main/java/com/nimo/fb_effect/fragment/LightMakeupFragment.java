 ...
 * 功能——轻美妆
 */
public class LightMakeupFragment extends FBBaseFragment {
    ...
    private int currentPosition = FBSelectedPosition.POSITION_LIGHT_MAKEUP;
...
//        returnIv = view.findViewById(R.id.return_iv);
        ...
        //添加标题
        htTabs.clear();
        htTabs.add(requireContext().getString(R.string.beauty_light_makeup));
//        topIndicatorView.setHorizontalScrollBarEnabled(true);
        ...
        FBSelectedPosition.POSITION_LIGHT_MAKEUP = 0;

        FBEffect.shareInstance().setFilter(FBFilterEnum.FBFilterBeauty.getValue(), "",0);
        RxBus.get().post(FBEventAction.ACTION_SYNC_LIGHT_MAKEUP_ITEM_CHANGED, "");
...
//                ((AppCompatTextView) convertView).setText(htTabs.get(position));
            ...
            @Override public Fragment getFragmentForPage(int position) {
                Log.e("position:LightMakeup", position + "");
                return new LightMakeupItemFragment();
        ...
        htPager.setCurrentItem(FBSelectedPosition.POSITION_LIGHT_MAKEUP,false);
        changeTheme("");
    ...
    public void onDestroy() {
        super.onDestroy();
        FBUICacheUtils.setLightMakeupPosition(0);
        FBEffect.shareInstance().setStyle("",0);
    }
}
