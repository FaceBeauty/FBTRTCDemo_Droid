 ...
 * 美型的Item适配器
 */
public class BeautyBodyItemViewBinder
    extends ItemViewBinder<BeautyBody, BeautyBodyItemViewBinder.ViewHolder> {
  ...
  @Override protected void onBindViewHolder(@NonNull ViewHolder holder, @NonNull BeautyBody item) {

    //-50到50参数区间 ui逻辑为50 为初始值
    String key = item.name();

    int initialValue = 0;
    /*if (key.equals(BeautyBody.CHIN_TRIMMING.name()) ||
        key.equals(BeautyBody.FOREHEAD_TRIM.name()) ||
        key.equals(BeautyBody.EYE_SAPCING.name()) ||
        key.equals(BeautyBody.EYE_CORNER_TRIMMING.name()) ||
        key.equals(BeautyBody.NOSE_ENLARGING.name()) ||
        key.equals(BeautyBody.PHILTRUM_TRIMMING.name()) ||
        key.equals(BeautyBody.MOUTH_TRIMMING.name())
    ) {
      initialValue = 50;
    }*/

    holder.itemView.setSelected(getPosition(holder) ==
        FBUICacheUtils.beautyBodyPosition());
    ...
    holder.itemView.setOnClickListener(new View.OnClickListener() {
      @Override public void onClick(View view) {
        ...
        getAdapter().notifyItemChanged(FBUICacheUtils.beautyBodyPosition());
        FBUICacheUtils.beautyBodyPosition(getPosition(holder));
        getAdapter().notifyItemChanged(FBUICacheUtils.beautyBodyPosition());
        FBState.currentBeautyBody = item;
        //同步滑动条
        RxBus.get().post(FBEventAction.ACTION_SYNC_PROGRESS, "");
      }
    });
